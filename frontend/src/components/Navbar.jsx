...
const Navbar = ({ toggleSidebar, isOpen }) => {
  ...
  const { url, userData } = useContext(AppContext);
        ...
        <div className="w-full px-2 sm:px-30 h-[70px] flex justify-between items-center">
          ...
          {userData?.name ? (
            <div
              ...
              className="w-10 h-10 bg-blue-800 rounded-full cursor-pointer text-white text-3xl font-bold flex justify-center items-center"
            >
              {userData.name[0]}
            </div>
          ) : (
